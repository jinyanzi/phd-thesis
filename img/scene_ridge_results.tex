...
\centering
    ...
    \end{subfigure}%
    \caption{Scene learning results on the first topic in \ref{fig:scene-topics}: (\subref{subfig:scene-ridges}) shows multiple ridges learned from local maximal grid. (\subref{subfig:scene-perp-width}) is the perpendicular width, where lighter intensity indicates a larger width. (\subref{subfig:scene-entry-exit}) shows extracted entry/exit hotspots indicates by the green and red star. And the yellow arrow shows their direction.}
